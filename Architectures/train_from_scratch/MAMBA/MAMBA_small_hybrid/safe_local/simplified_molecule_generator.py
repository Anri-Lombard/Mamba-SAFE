...
def save_molecules_to_file(molecules, output_file):
    with open(output_file, 'w') as f:
        for molecule in molecules:
            f.write(f"{molecule}\n")
    logging.info(f"Generated molecules saved to {output_file}")

def evaluate_model(model_dir, tokenizer_path, n_samples, max_length, top_k, top_p, temperature, output_file):
    ...
    save_molecules_to_file(generated_smiles, output_file)

    print(f"Number of samples generated: {len(generated_smiles)}")
    print(f"\nFirst 10 generated SMILES (saved to {output_file}):")
...
if __name__ == "__main__":
    ...
    evaluate_model(args.model_dir, args.tokenizer_path, args.num_samples, args.max_length, 
                   args.top_k, args.top_p, args.temperature, args.output_file)
