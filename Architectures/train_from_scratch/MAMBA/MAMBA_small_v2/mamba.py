...
from safe.tokenizer import SAFETokenizer # Does not work with auto-tokenizer
...
class SAFEDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        safe_string = item['SAFE']

        # Encode the SAFE string
        encoding = self.tokenizer.encode(safe_string, ids_only=True)
        ...
        # Pad if necessary
        pad_length = self.max_length - len(encoding)
        if pad_length > 0:
            encoding = encoding + [self.tokenizer.pad_token_id] * pad_length
...
def train(model, data_loader, optimizer, criterion, device, max_grad_norm=1.0, DEBUGGING_IS_ON=False):
    model.train()
    total_loss = 0
...
def evaluate(model, data_loader, criterion, device, DEBUGGING_IS_ON=False):
    ...
    with torch.no_grad():
        for batch in data_loader:
            input_data = batch['input_ids'].clone().detach().to(device)  # data without downsized dimension for Mamba model
...
#             batched_input_ids.extend(inputs)
#         return torch.tensor(batched_input_ids)
...
#     vocab_size = 1180 # For SAFE tokenizer
...
safe_tokenizer = SAFETokenizer.from_pretrained("./tokenizer.json")
dataset_dict = DatasetDict.load_from_disk('../../Datasets/MOSES/datasets')
# tokenized_dataset = DatasetDict.load_from_disk('../../Datasets/MOSES/tokenized_datasets')

# Determine the maximum length of encoded SAFE strings in your dataset
max_length = max(len(safe_tokenizer.encode(item['SAFE'], ids_only=True)) for item in dataset_dict['train'])
print(f"Maximum sequence length in the dataset: {max_length}")

# You might want to round this up to the nearest power of 2 for efficiency
max_length = 2**math.ceil(math.log2(max_length))
print(f"Rounded max_length: {max_length}")

# Create datasets with the determined max_length
train_dataset = SAFEDataset(dataset_dict['train'], safe_tokenizer, max_length=max_length)
val_dataset = SAFEDataset(dataset_dict['test'], safe_tokenizer, max_length=max_length)
...
for epoch in tqdm(range(num_epochs)):  # loop over the dataset multiple times
    ...
    train_loss = train(model, train_loader, optimizer, criterion, device, max_grad_norm=10.0, DEBUGGING_IS_ON=debugging_is_on)
